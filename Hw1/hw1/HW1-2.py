...
def conv(imgTempff, imgSS, filSS, tempp, ii, jj, kk):
    ...
    temppSort = sorted(tempp)
    num = int(len(temppSort) / 2)

    if (ii + 1 < imgSS[0]) and (jj + 1 < imgSS[1]):
        print('imgTempf[', ii + 1, '][', jj + 1, '][', kk, ']: ', imgTempff[ii + 1][jj + 1][kk], ', temp[num]after: ', tempp[num], 'temppSort: ', temppSort)
        imgNew[ii + 1][jj + 1][kk] = temppSort[num]
    tempp.clear()
    temppSort.clear()
    return imgNew
