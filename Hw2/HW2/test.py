...
def conv(img_original_c, img_temp_c, img_size_c, fil, fil_size, temp, i, j):
    img_new_c = np.zeros(img_original_c.shape[0],img_original_c.shape[1])
    img_new_c = img_new_c.reshape(img_original_c.shape[0], img_original_c.shape[1])
    if fil_size[0] % 2 != 0:
